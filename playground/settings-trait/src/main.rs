...
mod settings {
    ...
    impl ItineraryEntry {
        pub fn new<T: SettingType>(setting_id: EntityId<Setting<T>>, ratio: f64) -> Self
        where
            T: SettingType,
            EntityId<Setting<T>>: SettingId,
        {
    ...
    pub trait SettingContextExt {
        ...
        -> P::Value;
        fn get_settings_members<T: SettingType>(
            &self,
            setting_id: EntityId<Setting<T>>,
        ) -> Vec<PersonId>;
...
type HomeId = EntityId<Setting<Home>>;
impl SettingId for HomeId {
    fn get_id(&self) -> usize {
        self.id() as usize
    }
    fn get_type_id(&self) -> TypeId {
        TypeId::of::<Setting<Home>>()
    }
}
...
impl SettingId for WorkId {
    fn get_id(&self) -> usize {
        self.id() as usize
    }
    fn get_type_id(&self) -> TypeId {
        TypeId::of::<Setting<Work>>()
    }
}
...
fn main() {
    ...
    context.register_property::<Setting<Home>, Alpha>();
    context.register_property::<Setting<Work>, Alpha>();
